...
| D64 | tooling | `scripts/parity.mjs` is method-blind | It matches a route by PATH only, so calling `GET /x` also counts `POST /x` as covered. That hid two uncalled routes while `apps/app` read 123/123: `POST /me/packages` (a provider could withdraw a bundle but never publish one) and `POST /sessions/:id/files` (files could be listed, never shared). Both are now built in `apps/app`. The script itself is unchanged — making it method-aware means matching each client's call syntax (`_api.post(` in Dart, `fetch(..., {method})` in TS, plus helpers like `_objects`) and will lower reported numbers, so it wants its own change and a new baseline. |
| D69 | web | **The web admin verification screen was a mock** — fixed for verification only | Until 2026-09-14 `/admin/verification` drew Approve/Refuse buttons wired to nothing, a tier picker the API has never accepted (a credential type carries its tier), and document links to `#`. No credential could be decided from the web at all, which is where CLAUDE.md puts admin work. It now runs the automated check, opens the document through the audited five-minute link, and verifies or refuses (a refusal needs a reason, which is emailed to the provider). **The rest of the web admin console is unchanged and unaudited here** — disputes rulings, safety claim/resolve and settlement still call nothing (`node scripts/parity.mjs --missing apps/frontend`). The web shell itself is still a preview build: fixed avatars, sample nav badges and a role switcher. |
