...
| D46 | M1 | `audit_log` exists; coverage is four actions, not all of them | Built (migration 0033, append-only by trigger, `AuditService` in `common/` so no module has to depend on `admin/` to record a decision). It now records credential verify/reject, dispute rulings, and family/domain manifest publishes — the human decisions someone would ask about later. **Not yet covered:** dispute settlement, moderation clears, escrow release and refund, and session recording consent. Also worth naming: **no code path writes `fee_schedules` at all** — a rate change is a manual SQL statement today, so there is nothing to audit and nothing to review either. |
