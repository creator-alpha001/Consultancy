...
| D44 | both | Client response types are not checked against the API | The enrolment screen read `res.recoveryCodes`; the API returns `{ codes }`. TypeScript was happy — the type was hand-written and simply described the wrong shape — so a confirm that genuinely succeeded (201, factor stored) left the user on a working form with no error and no way forward. This is the **fourth** instance, and the third was the worst: the web's `Evaluation` type declared `dimensions`, the API never sent it, and `.map` on undefined made every completed engagement page a 500 — for both parties, so a seeker could not see their marks and a mentor could not see what they had returned. It sat there unnoticed because no test and no journey had ever opened a page for an engagement that had actually been marked. The others: `agreedPricePaise` vs `amountPaise`, and `listAboutUser` omitting `reply`. Hand-written client types are assertions about the API that nothing verifies. A generated client, or contract tests that exercise the shape, would close it; nothing else will. |
| D45 | safety | `safety/` is one third of what CLAUDE.md scopes it to | The module is defined as "reports, distress escalation, contact-leak detection". Only detection exists, as regexes (already recorded as a placeholder). **There is no user-facing reporting at all** — no table, no endpoint, no screen — so a seeker who is harassed, or sent something abusive in a session, has no way to tell anyone. Distress escalation works because screening flags text on the way in; nothing handles a person deciding for themselves that something is wrong. For a platform whose duty-of-care rules (#24–27) are treated as correctness requirements, this is the largest remaining hole in the product. |
| D46 | M1 | `audit_log` does not exist | CLAUDE.md hard rule #14 names it directly — "`audit_log` and ledger tables are append-only" — and there is no such table and no code that writes one. `auth_events` covers identity events only; nothing records who changed a fee schedule, published a manifest, ruled a dispute, or decided a credential. Those are exactly the actions a dispute or a regulator would ask about later, and the ledger's own append-only history does not cover any of them. Found by checking the schema against the hard rules rather than by anything failing. |
...
| `safety/`, `taxonomy/` | **Service layer only — no HTTP controllers.** `notifications/` left this row when the outbox relay was built; the moderation queue is now reachable through `board/`'s own controller. `taxonomy/` is internal by design. `safety/` is the one that matters — see D45 | Whichever milestone needs the screen |
