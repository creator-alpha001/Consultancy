...
| D40 | M4 | An endpoint can be broken for its whole life if nothing calls it | `GET /admin/credentials/queue` ordered by `created_at`, a column `provider_credentials` does not have, so it threw on every call from the day it was written. Nothing noticed: no screen called it and no test ran it. Fixed, with a test that asserts the oldest-first ordering the queue exists for — but the class of gap is the point, and other endpoints with no caller may be in the same state. Worth a sweep: every route with no client and no test is unverified, whatever the module's coverage says. |
