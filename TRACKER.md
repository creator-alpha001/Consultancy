...
Last updated: 2026-09-14 · **The web on the app's pattern, batch one (D73): one sign-in for both sides with the same "Get guidance / Give guidance" choice; a phone tab bar with the app's five tabs; every page checked at 360 px in a real browser (`apps/frontend/test/screens.mjs`, 56 pages, 0 problems). The mentor pages were rebuilt on real data — the dashboard, earnings and verification pages had been drawing invented figures (a 99% on-time rate over 412 pieces of work, ₹3,84,200 paid this year, a bank account, statements, a tiered 15/12/8% fee, a certificate under review) — and payout, languages, bundles, booking rules and days off now exist on the web.** Before that: **App UI pass at phone size (D72): one sign-in screen now offers "Get guidance" and "Give guidance", carried into sign-up; every screen is drawn at 360 dp (and with large text) from recorded API responses in `apps/app/test/screens`, which fails on any overflow and writes screenshots for review. That review found screens printing codes (`upsc_cse`, `settled_split`, `written qa`), offers shown as ₹0, a verification screen with blank skill names, a profile saying "Completed 0" beside a card saying 18, and a panel component that silently dropped its explanation whenever it had no title.** Before that: **The app was driven end to end on an Android emulator against the real API, as both a seeker and a provider (D71). That found nine defects no test had: one in the database (an engagement funded from a package could never start work — migration 0056) and the rest in the app, including a privacy one (a second account on the same phone saw the first account's engagements from memory) and a provider who could not open a board request to make an offer.** Before that, the same day: **The web's money path works end to end (D69): book a service, write and lock the agenda, confirm terms, pay into escrow, complete and release, raise/appeal/withdraw a dispute, review; post a board request, offer on it, award. Several pages that showed invented data (a seeker profile, a 15% fee split, sample dispute evidence) now show real data or say plainly what is not built. Web route coverage 95/152, from 60.** Before that, the same day: **The app's writes were broken in ~15 places behind 100% route coverage — fixed, and now guarded (D70): `scripts/contract-bodies.mjs` compares every client body with the API's declared body in CI, and an end-to-end test sends the corrected bodies through create → agree → agenda → lock → pay → submit. Found on the way: an agenda-item trigger that silently cancelled every delete (migration 0055), and agenda drafts that could only ever be saved once.** Earlier the same day: **Operations console made real (D69): dispute ruling and settlement, the safety queue and held board questions, the fee schedule and the audit log now come from the API — several of these screens had shown invented cases, figures and reviewer names. Second-factor secrets are encrypted at rest (D18). Route parity now checks the HTTP method (D64).** Before that, the same day: **Accounts completed (D20 mostly closed, D67–D69): forgot/reset password, email confirmation, password change, idle-expiring sessions, names and mentor profiles, seeker field selection, and Amazon SES email behind the outbox; readiness no longer assumes a family (a rule #1 violation, now fixed); the web admin can finally record a verification decision. See `docs/ACCOUNTS-AND-ONBOARDING.md`.** Earlier the same day: **Agora wired for live sessions (D65): voice-first calls and audio-only cloud recording in `apps/api` and `apps/app`, never yet run against a real Agora project; a recorder that idles out goes unnoticed (D66). Also closed: provider bundles and in-session file sharing in `apps/app`, hidden by a method-blind parity check (D64).** See `docs/VIDEO-SESSIONS.md`. Previously, 2026-09-09 · **`apps/app` exists: a Flutter client for seekers and providers, Slice 0 complete and driven against the real API.** The anti-drift spine is in — a generated 148-route inventory, a parity check wired into CI, and `packages/design/tokens.json` rewritten to carry the CURRENT design system (closing D36) with `apps/frontend` checked rather than generated. 85 Dart tests. Parity's first run found the web agenda page's controls are inert; see D56.
...
| — | **apps/app** (Flutter, unscheduled) | **Both journeys working end to end; no screens stubbed.** Upload, slot picking, bundle publishing and session file sharing built (see D51, D64) | n/a — see "apps/app — the Flutter client" below |
...
**Agora wired in (2026-09-14, D65), still unproven against Agora itself.**
`ROOM_PROVIDER=agora` selects `AgoraRoomProvider` (per-user tokens bound
to one channel and one uid) and `AgoraCloudRecordingProvider` (audio-only
composite recording straight into the private ap-south-1 bucket), and
`apps/app` joins through the Agora Flutter SDK: voice-first, dual-stream,
the SDK's own downlink fall-back to audio, automatic drop/return reports
to `/sessions/:id/connection`, and a switch to voice only after ~6s of
weak quality. Recording is now a `session_recordings` run (migration
0053) rather than only a flag. **None of this has touched a real Agora
project** — no account or credentials exist here, so the requests are
tested against the documented shapes with a fake HTTP client. M5 stays
partial: its bar is a real call on 3G, and that has still not happened.
...
**The assessment loop (added in the second pass).** The seeker's side
shows what they sent, the scores against the category's own dimensions,
the marks on the work and the action items they can tick off. The
provider's side scores against those dimensions and **offers no way to
add, rename or remove one** (#16) — comparability is the only reason the
scale exists. Annotations carry required body text, which is what a
screen reader is given: the work is usually a photograph of handwriting,
so a mark that exists only as a coordinate is not information.

**Trust (added in the third pass).** Reviews carry the family's own
dimensions — "told me the hard truth", "on time" — which are NOT an
assessment template: a template grades the work against a category
rubric (#16), these describe what the person was like to work with. A
family that declares none gets a single overall rating and the screen
renders exactly that, inventing no scale. The right of reply is there:
one, by the subject, append-only.

Disputes anchor a claim to the locked agenda — the screen asks which
goals, shows the original-language wording where what is displayed is a
translation (#20), and says plainly that raising a case does not make a
refund faster. The ladder is rendered from the family's `disputeTiers`
with no code switching on any rung, and appeal is offered only while the
family says a further rung exists.

**Nothing is stubbed.** The last seven screens landed together and
`lib/features/placeholder/` was deleted with them. Of note: the board's
request screen is where #15 becomes visible rather than merely obeyed —
its ordering control is driven by a `ProposalOrder` enum with no price
member, so adding one is an edit to that enum rather than a string
someone passes through, and the browser driver asserts on the rendered
control that no price ordering is offered.
...
- **Booking picks a slot.** `lib/features/session/book_session.dart`
  lists the server-computed slots from `/providers/:id/slots`; the client
  never evaluates an availability rule.
...
| D28 | M1 | Notification transports: **email now exists; push, SMS and WhatsApp do not** | **The money half was closed earlier** (the relay dispatches payouts and refunds). **Email closed 2026-09-14:** `notifications/email/` has a transport seam — `SesEmailTransport` (Amazon SES v2, ap-south-1) or `LoggingEmailTransport` (default; logs instead of sending, and never logs a link in production) — and the relay composes `identity.password_reset`, `identity.email_verification` and `verification.decided` at send time from an en/hi catalogue. A link that was superseded or expired while queued is closed as skipped, not sent. SES has not been exercised against a real AWS account. `escrow.held`, `payout.failed` and the rest still have no transport, so the relay leaves them pending and reconciliation reports them. |
...
| D64 | tooling | **Closed 2026-09-14** — `scripts/parity.mjs` is method-aware | It used to match a route by PATH only, so a client that read `GET /x` was credited with `POST /x`. That hid two unbuilt app features (`POST /me/packages`, `POST /sessions/:id/files`, both since built). `methodAt()` now reads the verb at each call site — the ApiClient method before a Dart path literal, or a `method:` in a TypeScript call's options (GET otherwise) — and an undeterminable call still counts, so the heuristic never invents a gap. Applied to the web it removed 8 false credits (`POST /engagements`, `/board/posts`, `/board/posts/:id/proposals`, `/engagements/:id/disputes`, `/engagements/:id/complete`, `/attachments`, `/reports`, `/sessions/:id/recording`) — all real gaps, each only linked to or read. The app stayed at 100%. Baseline rewritten. |
| D73 | web | **Fixed 2026-09-14 (batch one of the web parity work)** — the web now follows the phone app's pattern for sign-in, navigation and the provider's supply side, and stopped drawing invented data | **Sign-in:** the same one-screen "Get guidance / Give guidance" choice as the app, as plain links (no JavaScript needed); the code box only for the side that holds a second factor or when the API asks; a fresh sign-in lands on that role's own home, and a person who chose the other side is told which kind of account theirs is (`?notice=account-is-…`, shown by the shell). Tested in `auth.test.ts`. **Navigation:** below 768 px a bottom tab bar with the app's tabs (Home · Find · Work · Sessions · You / Dashboard · Requests · Work · Earnings · You); the account menu lost its "preview build — switch product" list and gained Sign out; the footer's "Preview build — not connected to the API" line is gone. **Invented data removed:** provider dashboard (on-time %, piece count, median reply, payout date, "top band" in search), earnings (year total, bank account, schedule, statements, fee table), services (a 15/12/8% fee — rates come only from `fee_schedule_at`, #8), verification (an "Environment and ecology" certificate with a decision date). **Built on real API data:** dashboard (`/me/readiness` blockers, work waiting with sent work first, `/me/earnings` figures, booked sessions, board), earnings (summary, payouts, per-engagement split, `/me/payout-destination`), `/provider/payout` (last four and IFSC only, #31), `/provider/languages` (#19), bundles on services (`/me/packages`), booking rules and days off on availability, per-skill history on verification (`/me/skill-stats`). **Wrong on the web too:** the credential form read inputs as `name`/`type` where the API sends `key`/`kind` (every field "vd_undefined", nothing reached the claim) and showed only the first type's inputs — now one section per type, with a skill checklist from the family; the seeker's list called `delivered` "waiting on you" with a "review window" and several pages said money "releases automatically" (the API has no automatic release); a provider opening a request saw every competing offer and an "Award" button — now only their own offer, or the form to make one (#15); status chips printed codes. **Layout:** a panel in a grid grew to its widest child and pushed a 360 px page 339 px sideways; a 64-character hash did the same on every locked agenda; a server-action form carried `target="_blank"`, which React drops, breaking hydration. **Still open on the web:** file upload (the credential type that needs an attachment id cannot be submitted), the session room, in-session chat, free questions, review replies, bundles for seekers. The monthly-statement route `/provider/earnings/[id]` is no longer linked and reads the seeker's money feed; to be replaced. |
| D72 | clients | **Fixed 2026-09-14** — the app UI at phone size, and the response fields screens read under the wrong names | **Sign-in** is one screen for both: a "Get guidance / Give guidance" choice changes the wording and where "create an account" leads (`/register?as=provider` pre-selects the role); it grants nothing — the account the API returns decides which app opens (#28), and a person who picked the other side is told which kind of account theirs is. Show-password toggle added. **Harness:** `apps/app/test/screens/screens_test.dart` draws every seeker, provider and signed-out route at 360×640, at 360 wide with text scaled 1.3, and on a tall 360 canvas, from responses recorded off the dev API by `apps/app/tool/screens/record.mjs` (the test lists any request it has no recording for). `SCREENSHOTS=1` writes a PNG per screen. Nothing overflowed; what the pictures showed did: **read under the wrong name** — an offer's `proposedAmountPaise` (every offer ₹0), a profile's `trackRecord.completedEngagements`, credential status `under_review` ("Unknown"); **missing from the API** — skill names on `GET /me/skill-stats`, credential type names on `GET /me/credentials`, the provider's name on `GET /board/posts/:id/proposals` (all added, asserted in `reputation.e2e` and `client-request-bodies.e2e`); **codes on screen** — engagement types, price bands and calendar phases on a field page, escrow states on the money screen, domain codes on verification. **Kit:** `Panel` drew its note only when it also had a title, so every untitled panel lost its explanation; helper and error text was cut to one line; every "selected" state (tab, chip, segment) was a slate colour from nowhere, now the brand's soft tint; tab labels were "Provider" and "Engagement", now Home · Find · Work · Sessions · You and Dashboard · Requests · Work · Earnings · You. Recovery codes are no longer offered to seekers, who hold no second factor; a provider's "Your setup" now reaches languages, payout and training. **Known artefact:** the screenshots show a thin black edge on elevated buttons — flutter_test draws shadows that way; a phone does not. |
| D71 | clients | **Fixed 2026-09-14** — driving the app on a device found nine defects the tests did not | Seeker and provider, one Android 16 emulator, the dev API. **Database:** `try_promote_engagement_to_working` ran when the escrow was held or the agenda locked, only if the engagement was already `agreed`. A package draw holds escrow while it is still `draft`, so held → locked → agreed left it `agreed` for ever, paid and locked and refusing the seeker's work. Migration 0056 makes agreeing a promotion event too and repairs stranded rows; tested both ways in `payment.e2e.spec.ts` (#12 still holds: agreeing without a lock does not promote). **App, privacy:** every fetch was cached in memory across sign-out, so the next account on the phone was shown the previous account's engagements (the API was right, #28; the app never asked again). The repository is now rebuilt when the signed-in account changes, which discards everything; `test/session/account_switch_test.dart` fails without it. **App, statuses:** the engagement status list was invented — `in_progress`, `in_review`, `revision`, `awaiting_payment`, none of which the API sends — and lacked `working` and `assessed`, which it does. So work under way rendered as unknown, a provider's dashboard said "nothing waiting" with work waiting, and a seeker was told to "check the work" at `delivered` (which means it is with the provider) and offered a Complete the API refuses until `assessed`. **App, provider side:** the engagement screen and work list were written for the seeker only (a provider saw their own name as the other party, "Hold the money", "Your money has left your account"); now each role gets its own next step and escrow wording. A provider tapping an open request was bounced to their dashboard — the redirect let providers into an allow-list of screens that omitted the board, free questions, reporting and legal — so a provider could not make an offer from the app; it is now a deny-list of seeker-only screens, tested in `test/routing/bounce_test.dart`. Provider tabs drew a back arrow (nested routes). **App, forms:** seven submit buttons never enabled because their text fields did not rebuild the screen — including the two-factor confirm button, so **no one could finish enrolling a second factor in the app**, plus note-only work submission, posting a request, raising and appealing a dispute, replying to a review and annotating. **App, copy and safety:** "The goals were met" released money on one tap with no confirmation; saving a draft agenda gave no sign it had saved; a board list kept showing "0 offers" after an offer. **Not fixed, noted:** `POST /engagements/:id/agree` and `POST /agendas/:id/lock` each complete on ONE party's call, though the copy says "both of you confirm" — a provider can end up bound by goals they never saw locked. That is a product decision, not a bug fix. |
| D70 | clients | **Fixed 2026-09-14** — the app sent the wrong request body to about fifteen routes, behind 100% route coverage | Found by reading each controller's `@Body()` beside each repository call. All of these were silently broken, i.e. rejected by the API or dropped on the floor: creating an engagement and a board post (no `currency`, a NOT NULL column); saving an agenda (`{language, items:[{ordinal,text}]}` where the API has only ever read `{originalLang, expectedDeliverable, successCriteria, items:[{labelLang,labelText}]}`); attaching a file to a submission (`attachmentIds[]` vs `attachmentId`); scoring (a `scores[]` list vs one dimension per call); annotating (`body/x/y/isActionItem` vs `bodyText/bodyLang/anchorX/anchorY`); booking a session (`durationMinutes` vs a required `scheduledEnd`); making an offer (`amountPaise` vs `proposedAmountPaise`) and reading its result as an engagement when the API returns the proposal; asking a question (`body/language` vs `bodyOriginal/bodyLang`); the payout account (`bankIfsc` vs `ifsc`); submitting a credential (by an `id` credential types do not have, with no `skillCodes`, reading field lists under keys that do not exist — so the form showed a single invented `reference` field); reports (`detail` vs `detailOriginal/detailLang`); session extensions (no `amountPaise`, which the API requires); availability exceptions (`date/available` vs `onDate`); discounts (`amountPaise` vs `discountPaise`). All corrected in `apps/app`, with the UI each needed (agenda deliverable and success criteria, a skill picker for credentials, a price for an extension). **Also fixed server-side:** saving an agenda draft a second time hit the one-active-agenda index and returned a 500 — drafts now replace in place while unlocked; malformed agenda bodies answer `AGENDA_INVALID` instead of a 500; a submission may be a written note alone; and `check_agenda_items_immutable_once_locked` returned `NEW` on DELETE, which Postgres treats as "skip this row" — every agenda-item delete had been silently cancelled since 0011 (migration 0055). **Guarded now:** `scripts/contract-bodies.mjs --check` (CI and `dev.sh test`) fails when a client sends a key an inline `@Body()` type does not declare, or omits a required one — verified against a reintroduced mistake; `test/contract/client-request-bodies.e2e.spec.ts` sends the corrected bodies through the real API. **Limits:** the static check reads literals only — bodies built by helpers or typed as named interfaces are skipped, and nothing here checks RESPONSE shapes (D57). |
| D69 | web | **The web is usable end to end for the core money path; sessions and provider supply are still app-only** | 2026-09-14. Many web screens drew buttons that called nothing, and several drew invented data as if it were live — worse than drawing nothing, because people act on it. **Operations console:** verification (automated check, audited document link, verify/refuse with reason), disputes (the raised text in its original language, the API's evidence packet, prior rulings; rule with reasons and a split amount in whole rupees without floating point; settle as a separate step), safety (claim and resolve with a note; publish held board questions), configuration (`GET /admin/fee-schedules` through `fee_schedule_at`, `GET /admin/audit-log`). **Seeker and provider:** booking (`/book/:id` creates the engagement at the service's published price, with a category), the agenda (draft saved as often as needed, locked behind an explicit confirmation), the engagement hub's draft and agreed states (confirm terms, the two preconditions of #12 shown as steps, pay into escrow with an idempotency key minted with the page, cancel), completion, raising a dispute (claimed goals quoted into the statement in their locked words — there is no item-id field), appeal and withdraw, reviews against the family's own dimensions, the board (post with a budget range, offer, award straight to the new engagement's agenda), and reports using the family's own reasons. **Removed because they were invented:** sample dispute statements and evidence, a sample "automated summary", a hardcoded fee table and audit log with named reviewers, fixed nav badge counts, a fake seeker history on open requests, a hardcoded 15% fee split, a hardcoded report-reason list, and a "two-factor is on" claim. **Controls with no API are now explained instead of drawn:** revision requests, change orders, messaging outside a session, questions before awarding, returning a marked-up file on the web. **Still open on the web:** the session room (D63), provider supply screens (services, packages, availability, languages, payout, earnings), file upload (D51), and in-session chat. `node scripts/parity.mjs --missing apps/frontend` is the list. |
| D68 | domains | **Fixed** — readiness and training defaulted to one exam family in core code | `GET /me/readiness` defaulted to `upsc_cse` and `/me/training` to `civil_services_exams` when a client named none — hard rule #1's exact prohibition, in the endpoint every new provider hits first. Now the families come from the provider (signup family, credential domains, verified-skill domains, via `ReadinessService.familiesFor`), the minimum tier is read per family, training must be complete in every one, and training answers 422 `FAMILY_REQUIRED` rather than guess. Both clients now pass the family. |
| D67 | identity | **Two-factor for providers is still switched OFF** | Restated because the account work touched every sign-in path: `mfa_policy.provider = false` (migration 0039, at the repo owner's request so demo mentors can sign in). CLAUDE.md #32 requires it. Before launch: `UPDATE mfa_policy SET mandatory = true WHERE role = 'provider';` — the enrolment flow, QR code (now in the app and web), recovery codes and ticket all work and are tested. |
| D66 | sessions | A recorder that exits on its own is not noticed until someone stops it | Agora's recorder leaves after 120s with nobody publishing (`maxIdleTime`) or when its resource expires. Nothing listens for that — Agora's notification callbacks (NCS) are not wired — so `sessions.recording_active` stays true and the run stays open until a stop is requested, which then closes it cleanly (a 404 on stop is read as "already exited"). The practical case: both people step away for over two minutes mid-session, come back, and the badge says "Recording audio" while nothing is. Needs an NCS webhook route that closes the run and clears the flag, verified against a real project. Also unbuilt: nothing yet links a run's files into dispute evidence, and nothing deletes them when `recording_retention_until` passes. |
| D65 | sessions | **Decided 2026-09-14: Agora, voice-first, audio-only recording** | Chosen by the user after a cost comparison: at list price a 60-minute two-person session with composite VIDEO recording is ~$0.84 on Agora, audio-only recording ~$0.57, and a voice call with audio recording ~$0.21. So sessions join with the camera off (video a choice), and the recorder takes audio only (`streamTypes: 0`) — the evidence a dispute reads is the audio plus the locked agenda and the chat log. Agora's data-routing restriction to India (`areaCode`) is **not** set: it would fail anyone joining from abroad, and whether recordings and media must stay in India is a DPDP/legal question, not a code default. Separately found and fixed while wiring it, all of which meant the room could never have worked against the real API: `apps/app` sent consent as `{granted}` (API requires `{consentGiven}`, so every consent was a 400); read `mode == 'audio'` (the API stores `audio_only`); reported drops as `{lostSeconds}` (API requires `{state}`); read `GET /sessions/:id` as a flat session when it was nested (now both, additively); and its release Android manifest had no `INTERNET` permission. |
| D63 | sessions | **Video is wired in `apps/app` only (D65); the web room still does not reach the API** | The seam is real everywhere and the vendor is nowhere. The API's `HundredMsSandboxRoomProvider` returns `sandbox_room_<id>` with no network call; `apps/app`'s `FakeRoomClient` connects to nothing. That much was known and is the M5 decision (managed SFU, do not build one). What was NOT recorded is the state of the web room: `apps/frontend/src/app/sessions/[id]/session-room.tsx` is 286 lines of local React state that **calls no API at all** — consent is a `useState` that never reaches `POST /sessions/:id/consent`, "Join" sets a boolean, and the video area is a styled dark rectangle. So the consent record CLAUDE.md #21 turns on does not exist for any session conducted through the web app, and neither does the refusal that shifts the evidentiary burden. `apps/app` does hit all of those endpoints for real. **In-session chat** has the same shape: the API has served `GET/POST /sessions/:id/messages` since M5, `apps/app` now uses it, and `apps/frontend` makes no session-message call anywhere. Separately, `apps/frontend` has an `/engagements/[id]/messages` page describing engagement messaging that **has no API behind it at all** — only session-scoped chat exists. |
| D61 | disputes | A dispute's `reason_code` has no declared taxonomy — anywhere | `POST /engagements/:id/disputes` requires a `reasonCode`, and it is a free string: no enum, no CHECK constraint, and no family declaration. That is inconsistent with how the neighbouring concept is handled — `reportReasons` IS family manifest data, and the comment on `ReportReasonInput` gives the reason plainly ("a music-instruction family's list of things worth reporting is not an exam family's"). The same argument applies at least as strongly to disputes, which move money. **Neither client had ever sent one**: the web dispute page is unwired (D56) and `apps/app` did not exist. `apps/app` now sends a single neutral `agenda_not_met` and carries the substance in the body — the claimed goals plus an explanation — rather than inventing a taxonomy, because CLAUDE.md says to ask rather than invent on anything touching money. **This needs a product decision**, and until it is made two clients can send different strings for the same thing and nothing will notice. The likely answer is that dispute reasons become family manifest data beside `reportReasons` and `disputeTiers`. |
| D62 | app | A dispute claim's selected goals are prose, not rows | The screen asks which locked agenda items the claim is about — the right question, and the one the web page also asks — but the API has no field for the answer, so the selection is written into the dispute body as text. That is enough for a human adjudicator to read and is how the claim reads today, but it means the link between a dispute and the specific `agenda_items` it contests is not queryable, cannot be validated (nothing stops a body naming a goal from a different engagement), and cannot be shown back as structure in the admin queue. A join table would fix it. Worth doing before disputes carry real money. |
| D59 | both | **Fixed both sides** — an empty response body crashed a client, on exactly the path CLAUDE.md #3 protects | `GET /engagements/:id/assessment-template` answers with an **empty string** — not null, not a 404 — when the category has no template, which is the NORMAL case for an objective category. `/engagements/:id/evaluations/latest` and `/engagements/:id/disputes` do the same when there is nothing yet. The client's `_unwrap` returned `null as T` for an empty body, which is a `TypeError` the moment `T` is non-nullable — so the app died on "this kind of work has no scale to score against", the one state the rule exists to make renderable. Fixed by checking `null is T` and, where a body is genuinely required, raising `EMPTY_BODY` instead of pretending; `getOrNull` now requests `T?` so the absence is a value. List endpoints that can answer `""` go through `_objects`, which reads it as an empty list — "no disputes" and "an empty list of disputes" are the same fact. Found by probing the running API, not by reading types, which is the second time that has paid (see D58). **The API side is now fixed too:** `AbsentAsNoContentInterceptor` answers a GET whose result is nil with **204 No Content** rather than an empty 200. 204 is unambiguous, is what HTTP is for, and needed no client special-casing — `apps/frontend` already checked `status === 204` and so did `apps/app`. Scope is GET only: a mutation that returns nothing is not ambiguous, because its caller is not looking for a resource. Seven contract tests in `test/contract/absent-responses.e2e.spec.ts` pin it, including that a list is still `200 []` (an empty list is a complete answer, and 204-ing it would send every empty-state screen down its error path) and that an unauthenticated caller still gets 401 — 204 must mean "nothing to show you", never "you may not look". **Also fixed here:** `createTestApp` did not register the global interceptors, so no test exercised the response boundary at all — the bigint serialisation included. It does now, for the same reason the file already gives about the auth guard: a test app that serialises differently from production proves nothing about what a client receives. |
| D60 | app | The vocabulary grep missed banned words hiding in camelCase | `_identifierWord` used one lookbehind — `(?<![A-Za-z])word` — to stop "do**mains**" matching "mains". It worked, and it also meant `hasRubric` passed cleanly: "rubric" is preceded by "s", so the match was rejected. The word then went into real code, and the test that exists to prevent exactly that reported the file clean. Now two patterns: the original, case-insensitive, plus a **case-sensitive** `(?<=[a-z0-9])Word` for the camelCase form. They cannot be one regex — combining them under `caseSensitive: false` turns "preceded by a lowercase letter and capitalised" back into "preceded by any letter", which reintroduces the "domains" false positive. Both false results are recorded in the test's own comment, because the next person to simplify it will otherwise reintroduce one of them. |
...
| D18 | identity | **Closed 2026-09-14** — second-factor secrets are encrypted at rest | `identity/secret-box.ts`: AES-256-GCM, `v1:<keyId>:<iv‖tag‖ciphertext>`, key from `MFA_ENCRYPTION_KEY` (32 bytes base64 or a 32+ character passphrase), rotation through `MFA_ENCRYPTION_KEY_PREVIOUS`. The API refuses to start in production without a key. Secrets written before this are read as they are and re-sealed with the current key on their next use, so turning it on needs no bulk migration. **Ops still owns** where the key lives (a secrets manager or KMS filling the variable) and rotating it. |
| D20 | identity | **Mostly closed 2026-09-14** — reset, confirmation and idle timeout now exist | Migration 0054 and `identity/account.service.ts`: forgot/reset password (30-minute single-use link, all sessions revoked, 3 an hour, no account enumeration), email confirmation at registration (48 hours, re-sendable), password change (other sessions revoked), and sessions that expire when idle by role (`SESSION_POLICY`: 7 days for seekers and providers, 30 minutes for admins; absolute 30 days / 12 hours). Tokens are stored only as hashes and the link is derived at send time from `ACCOUNT_TOKEN_SECRET`, so none sits in the outbox. **Still open:** a seeker is not required to confirm before using the platform (a provider is, via readiness), and no account deletion or data export exists — both need a retention/legal decision. |
...
| `LoggingEmailTransport` | The DEFAULT when `EMAIL_TRANSPORT` is unset: emails are printed to the API log (with the link outside production) instead of sent. `EMAIL_TRANSPORT=ses` selects Amazon SES | Remains for dev and tests by design |
| `HundredMsSandboxRoomProvider` + `SandboxRecordingProvider` | The DEFAULT when `ROOM_PROVIDER` is unset: no network, no room, no recording, and the join credentials carry no token so `apps/app` uses its fake client. A real vendor (`ROOM_PROVIDER=agora`) now exists beside it and the two are never mixed; the API refuses to boot with `agora` selected and any credential missing | Remains for dev and tests by design |
...
| `sessions.mode = 'audio_only'` | Records the fall-back event. With Agora, `apps/app` now sets it from real network-quality reports (and the SDK adapts bitrate and falls a weak downlink back itself); with the sandbox it is still only set by hand | Proven only once a real call runs (D65) |
