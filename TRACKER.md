...
| D43 | M1 | A dead-lettered payout has no alert, only a report | After 8 failed attempts an event stops being retried, stays undispatched and keeps appearing in reconciliation. Nobody is told. For a payout that means a provider is owed money, the transfer was never instructed, and the only way anyone finds out is by looking at a report nothing runs (D23). The three gaps compound: no scheduler, no alerting, no relay tick — each individually survivable, together a way to not pay someone indefinitely. |
