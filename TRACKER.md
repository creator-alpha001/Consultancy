...
| D25 | web | Screens still missing: dispute detail, admin queues | Provider credential submission is now built (`/mentor/credentials`) and driven by `test/provider-journey.mjs`. What remains unbuilt: the dispute **detail** and appeal screens (raising one works, everything after it is blind), and the admin adjudication, credential-review and moderation queues — `/admin` covers reconciliation only. |
